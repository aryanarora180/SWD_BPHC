...
@HiltViewModel
class StudentConnectViewModel @Inject constructor(private val repository: AppDataSource) :
    ViewModel() {
    ...
    private val _connect = MutableLiveData<StudentConnect>()
    val studentConnect: LiveData<StudentConnect>
        get() = _connect

    fun loadConnect() {
        viewModelScope.launch(Dispatchers.IO) {
            _isDataLoading.postValue(true)
            when (val result = repository.getStudentConnect()) {
                is OperationResult.Success -> {
                    _connect.postValue(result.data)
                }
